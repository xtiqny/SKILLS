...
description: 聘才猫 - 面试出题大师 Use when calling Pincaimao Interview Question Master API to generate interview questions based on a job description and candidate resume. Requires PCM_API_KEY env var.
version: 1.0.1
allowed-tools:
  - Bash
metadata:
...
入口 `scripts/main.py`（自动完成简历上传 + 生成面试题）：

```bash
python3 scripts/main.py --resume <简历文件> --job-info "<JD全文>" [--job-title "<职位名>"] [--num 6] [--analysis false]
```

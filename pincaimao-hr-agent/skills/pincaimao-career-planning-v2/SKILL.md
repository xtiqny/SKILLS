...
description: 聘才猫 - 职业规划助手 V2 Use when calling Pincaimao Career Planning Assistant V2 API to generate career advice based on a resume and advice type. Requires PCM_API_KEY env var.
version: 1.0.1
allowed-tools:
  - Bash
metadata:
...
入口 `scripts/main.py`（自动完成简历上传 + 生成职业建议）：

```bash
python3 scripts/main.py --resume <简历文件> --type <初入职场|转型建议|晋升路径>
```
...
| `bot_id` | 是 | 固定为 `121182005294690304` |

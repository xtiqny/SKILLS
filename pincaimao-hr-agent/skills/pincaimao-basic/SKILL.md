...
description: 聘才猫平台基础能力 Use when calling any Pincaimao platform API — file upload, presigned URL, conversation list, message history, audio-to-text, resume JSON upload, or the base chat-bot-messages interface. Reference this skill before building any PCM agent skill.
version: 1.0.2
allowed-tools:
  - Bash
metadata:
  openclaw:
    ...
    requires:
      env:
        - PCM_API_KEY  # Shared API key for all platform endpoints.
      bins:
        - curl
        - python3
---
...
本 skill 提供平台通用接口的脚本实现，入口 `scripts/main.py`（仅依赖 Python 标准库）：

```bash
python3 scripts/main.py upload --file <文件路径>
python3 scripts/main.py presigned --cos-key "<cos_key>" [--expired 600]
python3 scripts/main.py conversations [--user <id>] [--limit 20]
python3 scripts/main.py messages --conversation-id <id> [--limit 20]
python3 scripts/main.py audio2text --cos-key "<cos_key>"
```

默认读取 `PCM_API_KEY`，也可用 `--env <ENV_NAME>` 指定其他环境变量名。下方为各接口的原始 curl 契约说明。
...
| 接口 | Key 要求 |
|------|----------|
| `chat-bot-messages` | 使用统一的 `PCM_API_KEY` |
| 其他所有接口 | 任意创建的 key 均可 |
...
```bash
# chat-bot-messages 示例（所有技能统一使用 PCM_API_KEY）
...
## 1. chat-bot-messages（智能体对话）

**POST** `https://api.pincaimao.com/agents/v1/chat/chat-bot-messages`
...
| 字段 | 必填 | 说明 |
|------|------|------|
| `bot_id` | 是 | 目标助手 ID，由具体智能体 skill 指定 |
| `query` | 是 | 依智能体而定 |
| `inputs` | 否 | 各智能体自定义参数 |
...
```bash
curl -N -s -X POST 'https://api.pincaimao.com/agents/v1/chat/chat-bot-messages' \
  -H "Authorization: Bearer $PCM_API_KEY" \
  -H 'Accept: text/event-stream' \
  -H 'Content-Type: application/json' \
  -d '{"bot_id":3022316191018873,"query":"...","inputs":{},"response_mode":"streaming"}' \
| python3 -c "
import sys, json
answer = ''
